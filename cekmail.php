...
if(isset($_POST["email"]))
{
    ...
    $hasil = mysqli_query($conn,"SELECT id FROM tb_user WHERE email='$_POST[email]'");
    $email = mysqli_num_rows($hasil);
     
    // Jika datanya diatas 0 (ada)
    if($email > 0) {
        echo '<font style=color:#F00>Email sudah terdaftar</font>'; // Tampilkan pesan
    }else{ // Jika belum ada
        echo '<font style=color:#06F>Email tersedia</font>'; // Tampilkan pula pesan
    }
}
?>
